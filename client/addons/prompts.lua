-- Credit: QBR-Core (multiple authors) @ https://github.com/qbcore-redm-framework/qbr-core/blob/main/client/prompts.lua
...
local function createPrompt(name, coords, key, text, options)
    if (Prompts[name] ~= nil) then
        print('[RPX] Warning: Prompt with name ' .. name .. ' already exists!')
    end
    Prompts[name] = {}
...
local function createPromptGroup(group, label, coords, prompts)
    if (PromptGroups[group] ~= nil) then
        print('[RPX] Warning: Prompt with name ' .. group .. ' already exists!')
    end
    PromptGroups[group] = {}
